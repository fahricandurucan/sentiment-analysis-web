...
export default function TimelinePage() {
  ...
  const getChartData = useCallback(() => {
      ...
      if (item[activeTab] === selectedOption) {
        ...
        if (!monthlyData[timeKey]) {
          monthlyData[timeKey] = {
            ...
            negative: 0,
            neutral: 0,
            date: date, // we sort by this later
          };
        }

        const sentiment = item.sentiment?.toLowerCase();
        if (sentiment && monthlyData[timeKey].hasOwnProperty(sentiment)) {
          monthlyData[timeKey][sentiment]++;
        }
      }
    });
    ...
    const positiveData = sortedMonths.map(
      (month) => monthlyData[month].positive
    );
    const negativeData = sortedMonths.map(
      (month) => monthlyData[month].negative
    );
          ...
          <div className="mt-8 w-full max-w-xs md:max-w-md mx-auto">
            ...
            >
              ...
              {dropdownOptions.map((option) => {
                let label = option;
                if (activeTab === "gender") {
                  if (option === "askmen") label = "Men";
                  else if (option === "askwomen") label = "Women";
                }
                if (activeTab === "generation") {
                  if (option === "millennials") label = "GenY/millennials";
                }
